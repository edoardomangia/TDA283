...
void Skeleton::visitArrSize(ArrSize *t) {} //abstract class
...
void Skeleton::visitLhsVar(LhsVar *lhs_var)
{
  /* Code For LhsVar Goes Here */

  visitIdent(lhs_var->ident_);

}

void Skeleton::visitLhsIndex(LhsIndex *lhs_index)
{
  /* Code For LhsIndex Goes Here */

  if (lhs_index->expr_1) lhs_index->expr_1->accept(this);
  if (lhs_index->expr_2) lhs_index->expr_2->accept(this);
...
void Skeleton::visitAss(Ass *ass)
{
  ...
  if (ass->lhs_) ass->lhs_->accept(this);
...
void Skeleton::visitIncr(Incr *incr)
{
  ...
  if (incr->lhs_) incr->lhs_->accept(this);
...
void Skeleton::visitDecr(Decr *decr)
{
  ...
  if (decr->lhs_) decr->lhs_->accept(this);
...
void Skeleton::visitArr(Arr *arr)
{
  ...
  if (arr->type_) arr->type_->accept(this);
...
void Skeleton::visitDoubBase(DoubBase *doub_base)
{
  /* Code For DoubBase Goes Here */


}

void Skeleton::visitBoolBase(BoolBase *bool_base)
{
  /* Code For BoolBase Goes Here */
...
void Skeleton::visitENew(ENew *e_new)
{
  /* Code For ENew Goes Here */

  if (e_new->basetype_) e_new->basetype_->accept(this);
  if (e_new->listarrsize_) e_new->listarrsize_->accept(this);
...
void Skeleton::visitELength(ELength *e_length)
{
  /* Code For ELength Goes Here */

  if (e_length->expr_) e_length->expr_->accept(this);
  visitIdent(e_length->ident_);
...
void Skeleton::visitNewDim(NewDim *new_dim)
{
  /* Code For NewDim Goes Here */

  if (new_dim->expr_) new_dim->expr_->accept(this);
...
void Skeleton::visitListArrSize(ListArrSize *list_arr_size)
{
  for (ListArrSize::iterator i = list_arr_size->begin() ; i != list_arr_size->end() ; ++i)
  {
    (*i)->accept(this);
  }
}
