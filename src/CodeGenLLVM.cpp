...
struct RVal {      // the result of an expression
  ...
  std::string blk; // label where v is defined ("" = current block)
};
...
struct CodeGen {
  ...
  void emitRuntimeDecls() {
    decls << "declare void @printInt(i32)\n"
  ...
  RVal genExpr(Expr *e) {
    ...
    if (dynamic_cast<ELitFalse *>(e))
      return {"0", "i1", ""};
    if (auto E = dynamic_cast<ELitInt *>(e))
      return {std::to_string(E->integer_), "i32", ""};
    ...
    if (auto E = dynamic_cast<EString *>(e)) {
      ...
      std::string p = F->t();
      F->emit("  " + p + " = getelementptr [" + std::to_string(L) +
              " x i8], [" + std::to_string(L) + " x i8]* " + g +
              ", i32 0, i32 0");
      return {p, "i8*", ""};
    }
    ...
    if (auto E = dynamic_cast<EAnd *>(e)) {
      // left operand
      RVal l = genExpr(E->expr_1);
      assert(l.ty == "i1");

      std::string LrhsStart = F->L(); // start of RHS evaluation if left is true
      std::string Lfalse = F->L();    // left was false
      std::string Ldone = F->L();     // merge point (result is defined here)

      // branch on left
      F->emit("  br i1 " + l.v + ", label %" + LrhsStart + ", label %" +
              Lfalse);
      ...
      // RHS branch
      F->emit(LrhsStart + ":");
      RVal r = genExpr(E->expr_2);
      assert(r.ty == "i1");

      // If r.blk is non-empty, the value r.v is defined in that block (like
      // inner &&/||), otherwise it's defined in LrhsStart.
      std::string Rblk = r.blk.empty() ? LrhsStart : r.blk;

      // We are currently at the end of Rblk, so just jump to Ldone
      F->emit("  br label %" + Ldone);
      ...
      // The result is defined in Ldone
      return {res, "i1", Ldone};
    }

    // logical || with short-circuit
    if (auto E = dynamic_cast<EOr *>(e)) {
      // left operand
      RVal l = genExpr(E->expr_1);
      assert(l.ty == "i1");

      std::string Ltrue = F->L();     // left was true
      std::string LrhsStart = F->L(); // evaluate right only if left is false
      std::string Ldone = F->L();     // merge point

      // branch on left
      F->emit("  br i1 " + l.v + ", label %" + Ltrue + ", label %" + LrhsStart);
  ...
  void genStmt(Stmt *s) {
    ...
    if (auto S = dynamic_cast<Decl *>(s)) {
      ...
      for (Item *it : *S->listitem_) {
        if (auto ni = dynamic_cast<NoInit *>(it)) {
          std::string a = F->t(); // unique SSA name
          F->emit("  " + a + " = alloca " + T);
    ...
    if (auto S = dynamic_cast<CondElse *>(s)) {
      ...
      popScope();
      bool thenTerm =
          F->blockTerminated; // remember whether THEN ended with ret/branch
      if (!thenTerm) {
        F->emit("  br label %" + Le);
      }

      // Reset for codegen of ELSE branch
      F->blockTerminated = false;
      ...
      popScope();
      bool elseTerm =
          F->blockTerminated; // remember whether ELSE ended with ret/branch
      if (!elseTerm) {
        F->emit("  br label %" + Le);
      }

      // After the if-else:
      // - If both terminate, the whole statement is terminated
      // - Otherwise, we need the join block Le
  ...
  void emitFunction(FnDef *f) {
    ...
    for (size_t i = 0; i < formals.size(); ++i) {
      std::string T = ptys[i];
      std::string a = local.t(); // unique SSA name, e.g. %t0, %t1...
    ...
    auto B = dynamic_cast<Block *>(f->blk_);
    assert(B);
    for (Stmt *x : *B->liststmt_)
      genStmt(x);

    if (local.retTy == "void" && !local.blockTerminated)
      local.emit("  ret void");
    local.body << "}\n\n";
    funs << local.body.str();
    popScope();
