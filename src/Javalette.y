...
%type <expr_> Expr6
%type <arrsize_> ArrSize
%type <listarrsize_> ListArrSize
%type <expr_> Expr5
%type <expr_> Expr4
%type <expr_> Expr3
%type <expr_> Expr2
...
;
Lhs : _IDENT_ { $$ = new LhsVar($1); }
  | Expr6 _LBRACK Expr _RBRACK { $$ = new LhsIndex($1, $3); }
;
...
Stmt : _SEMI { $$ = new Empty(); }
  ...
  | Lhs _EQ Expr _SEMI { $$ = new Ass($1, $3); }
  | Lhs _DPLUS _SEMI { $$ = new Incr($1); }
  | Lhs _DMINUS _SEMI { $$ = new Decr($1); }
...
Type : _KW_int { $$ = new Int(); }
  ...
  | _KW_void { $$ = new Void(); }
  | Type _LBRACK _RBRACK { $$ = new Arr($1); }
;
BaseType : _KW_int { $$ = new IntBase(); }
  | _KW_double { $$ = new DoubBase(); }
  | _KW_boolean { $$ = new BoolBase(); }
;
...
Expr6 : _IDENT_ { $$ = new EVar($1); }
  ...
  | _KW_new BaseType ListArrSize { std::reverse($3->begin(),$3->end()) ;$$ = new ENew($2, $3); }
  | Expr6 _LBRACK Expr _RBRACK { $$ = new EIndex($1, $3); }
  | Expr6 _DOT _IDENT_ { $$ = new ELength($1, $3); }
  | _LPAREN Expr _RPAREN { $$ = $2; }
;
ArrSize : _LBRACK Expr _RBRACK { $$ = new NewDim($2); }
;
ListArrSize : ArrSize { $$ = new ListArrSize(); $$->push_back($1); }
  | ArrSize ListArrSize { $2->push_back($1); $$ = $2; }
;
